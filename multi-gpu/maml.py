...
class Learner(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.device = torch.device(f'cuda:{self.gpu_id}' if torch.cuda.is_available() else 'cpu') # 이렇게 설정해도 multi-gpu로 돌아감
    ...
    def forward(self, batch_tasks, training=True):
        ...
        for task_idx, task in enumerate(batch_tasks):
            ...
            fast_model = nn.DataParallel(fast_model) # 추가 (multi-gpu)
            ...
            for i in range(0,num_inner_update_step):
                logger.info(f'num_inner_update_step: {i+1}') # 추가
                all_loss = []
                for inner_step, batch in enumerate(support_dataloader):
                    logger.info(f'checkpoint 1 ({inner_step+1})') # 추가
                    
                    batch = tuple(t.to(self.device) for t in batch)
                    logger.info(f'batch: {batch}') # 추가
                    input_ids, attention_mask, segment_ids, label_id = batch
                    logger.info(f'checkpoint 2') # 추가
                    outputs = fast_model(input_ids, attention_mask, segment_ids, labels = label_id) # 여기서 안 넘어감
                    logger.info(f'outputs: {outputs}') # 추가
                    
                    loss = outputs[0]      
                    logger.info(f'loss: {loss}') # 추가       
                    loss.backward()
                    inner_optimizer.step()
                    inner_optimizer.zero_grad()
                    
                    all_loss.append(loss.item())
                    logger.info(f'all_loss: {all_loss}') # 추가
